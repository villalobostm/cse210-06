...
class SceneManager:
    ...
    def prepare_scene(self, scene, cast, script):
        if scene == MENU:
            ...
            self._prepare_credits(cast, script)
        elif scene == NEW_GAME:
    ...
    def _prepare_menu(self, cast, script):
        ...
        script.add_action(INPUT, ChangeSceneClickAction(self.MOUSE_SERVICE, BUTTON_1_GROUP, NEW_GAME))
        script.add_action(INPUT, ChangeSceneClickAction(self.MOUSE_SERVICE, BUTTON_2_GROUP, HELP))
        script.add_action(INPUT, ChangeSceneClickAction(self.MOUSE_SERVICE, BUTTON_3_GROUP, CREDITS))
    ...
    def _add_button(self, cast, button_group, y_position, x_position, width, height, image):
    ...
    def _add_help_output_script(self, script):
        script.clear_actions(OUTPUT)
        script.add_action(OUTPUT, self.START_DRAWING_ACTION)
        script.add_action(OUTPUT, self.DRAW_BUTTON_4_ACTION)
        script.add_action(OUTPUT, self.END_DRAWING_ACTION)

    def _add_credits_output_script(self, script):
        script.clear_actions(OUTPUT)
        script.add_action(OUTPUT, self.START_DRAWING_ACTION)
        script.add_action(OUTPUT, self.DRAW_BUTTON_5_ACTION)
        script.add_action(OUTPUT, self.END_DRAWING_ACTION)